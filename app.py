...
# Build/load index (non-fatal if empty)
vi = VideoIndex()
INDEX_LOADED = False
try:
...
def fetch_latest_youtube_urls(channel: str, limit: int = 12) -> list[str]:
    """
    channel: a handle like '@JoshLloyd48' OR a full channel URL.
    returns: list of https://www.youtube.com/watch?v=... (newest first)
    """
...
def admin_ingest(x_admin_token: str = Header(default="")):
    """Ingest URLs from the JOSH_URLS env var. One-time build or append."""
